...
/////////////////////////////////////////////////////////
	...
	/** calculates  power of the Laplacian of order n
...
*/
	//static double qs=(5.0-sqrt(10))/15.0;
	static final double TWOOPI=sqrt(PI);
	
	/*
	 * s=max (log(N)/log(2),3.3219)
tm=min(N,4)

t=linspace(0+1/2^N,tm,N);
f=2*sqrt(pi)*s;
x=s./t;
x32=x.^(3/2);
y=exp(-x/4).*x32/f;
	 */
	public static float[] gaussT1D(int width) {
		...
		if (debug)  
			System.out.print(" \n");
		final float sigma= (float) max (log(width)/log(2.0),3.3219);
		double eps=  pow(0.5,width);
		float tm=(float) (min(width,4.0)); 
		double[] t=linspace(0+eps,tm,width);
		
		for (int u=0; u<width; u++) {
		 	
			final double x2=sigma/t[u];

			kernel[u]=(float)(exp(-0.25*x2)*pow(x2, 1.5)*0.5/TWOOPI/sigma);
			if (debug) 
				System.out.print(x2 +" ");
		}
		if (debug)  
...
public static float[] diffgaussT1D(int width) {
		...
		for (int u=0; u<width; u++) { 
			...
			kernel[u]=(float)((x2-6.0)*exp(-0.25*x2)*pow(x2, 2.5)*0.125/TWOOPI)/sigma/sigma;
			if (debug) 
				System.out.print(x2 +" ");
		}
		if (debug)  
