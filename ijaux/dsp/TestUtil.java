...
public class TestUtil {
	...
	public static<A> double corrcoef(A x, A y) {
		try {
			...
			if (n!=n1) 
				throw new IllegalArgumentException();
			double sum_x=0, sum_y=0,sum_x2=0, sum_y2=0, sum_xy=0, xi, yi;
			
			for (int i=0; i<n; i++) {
				xi=ax.elementDouble(i);
				yi=ay.elementDouble(i);
				sum_x+=xi;
				sum_y+=yi;
			...
			if (r==0)
				return 0;
			r/=(Math.sqrt((sum_x2 - n* sum_x*sum_x)*(sum_y2 - n* sum_y*sum_y)));
	 ...
	 public static void printcvector(float[] data) {
			for (int i=0; i<data.length; i+=2) {	
				if (data[i+1]>=0)
					System.out.print( "( "+data[i]+"+ "+data[i+1]+"*%i  ), ");
				else 
					System.out.print( "( "+data[i]+" "+data[i+1]+"*%i  ), ");		
			}
