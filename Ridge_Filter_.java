 ...
 *
 *
 * @contents The plugin computes the eigenvalues of the ridge filter
 * 
 * 
...
public class Ridge_Filter_ implements ExtendedPlugInFilter, DialogListener {
	...
	public final static String SIGMA = "WEING_sigma", MAX_LEN = "W_MAX", FULL_OUTPUT = "WFull_out", LEN = "W_len";

	// @AFilterField(key=LEN, value="initial scale")
	public int sz = Prefs.getInt(LEN, 6);
	...
	/**
	 * This method is to setup the PlugInFilter using image stored in ImagePlus and
	 * arguments of filter
	 */
	@Override
	...
	/*
	 * @Override public String helpInfo() { return "s=sigma^2 - scale; \r\n " +
	 * "normalized - scale by sigma; \r\n " +
	 * "separable - keep enabled for faster run;"; return getHelpResource(); }
	 */
	/*
	 * (non-Javadoc)
	 * 
	...
	private ImageStack filter(ImageProcessor ip, GScaleSpace sp, ImageStack imageStack) {
		...
		FloatProcessor luv = new FloatProcessor(width, height); // Hessian determinant
		...
		for (int i = 0; i < width * height; i++) {
			...
			fr=Math.pow(fr, 1.5);
			final double ee1 = 0.5 * (trace + disc)/fr;
			final double ee2 = 0.5 * (trace - disc)/fr;

			// non scaled
			//double guu = gx * gx * gyy  +gy * gy * gxx  - 2. * gx *gy *gxy;
			//double gvv = gx * gx * gyy  +gy * gy * gxx  + 2. * gx *gy *gxy;
		...
		imageStack.addSlice(fkey + "_Trace_" + sz, luv);
		luv.resetMinAndMax();
	...
	public static void main(String[] args) {
		...
		String urlstr="https://imagej.net/media/plugins/cnt-ridge-detection-original.png";
		IJ.open(urlstr);
